...
begin
    ...
    if open_shift_id is not null then
        ...
        set created_at = '2026-04-24T00:00:00Z'::timestamptz
        where id = open_shift_id;

        raise notice 'Updated open shift % for CAFE_LI_BAR to 2026-04-24.', open_shift_id;
        return;
    end if;
    ...
    update public.shifts
    set created_at = '2026-04-23T00:00:00Z'::timestamptz
    where id = latest_closed_shift_id;

    raise notice 'Updated latest closed shift % for CAFE_LI_BAR to 2026-04-23 so the next shift opens on 2026-04-24.', latest_closed_shift_id;
end $$;

...
begin
    ...
    if target_branch_id is null then
        raise exception 'Branch CAFE_LI was not found.';
    end if;
    ...
    if open_shift_id is not null then
        ...
        set created_at = '2026-05-11T00:00:00Z'::timestamptz,
            shift_date = '2026-05-11'
        where id = open_shift_id;

        raise notice 'Updated open shift % for CAFE_LI to 2026-05-11.', open_shift_id;
        return;
    end if;
    ...
    if latest_closed_shift_id is null then
        raise exception 'No open shift or closed seed shift was found for CAFE_LI.';
    end if;

    update public.shifts
    set created_at = '2026-05-10T00:00:00Z'::timestamptz,
        shift_date = '2026-05-10'
    where id = latest_closed_shift_id;

    raise notice 'Updated latest closed shift % for CAFE_LI to 2026-05-10 so the next shift opens on 2026-05-11.', latest_closed_shift_id;
end $$;
